				...
				if (e.type == EventType.MouseDrag && e.button == 1 && toolbarInt == 2)
				{
					...
					if (Physics.Raycast(ray, out terrainHit, 200f, hitMask.value))
					{
						...
						for (int j = 0; j < positions.Count; j++)
						{
							...
							if (dist <= brushSize)
							{
								colors[j] = (new Color(AdjustedColor.r + (Random.Range(0, 1.0f) * rangeR),
									AdjustedColor.g + (Random.Range(0, 1.0f) * rangeG), AdjustedColor.b + (Random.Range(0, 1.0f) * rangeB),
									1));
